...
class DailyImageLandscapeProvider : AppWidgetProvider() {
    ...
    private fun updateAppWidget(
        ...
        thread {
            try {
                // Try cache first
                val cached = WidgetCacheManager.getAndPopCachedImage(context, isLandscape = true)
                if (cached != null) {
                ...
                // If cache is empty or fails, fall back to direct request (with landscape filtering)
                ...
                if (conn.responseCode == 200) {
                    ...
                    if (jsonArray.length() > 0) {
                        ...
                        // If we didn't find any landscape image in shuffled list, do a simple random fallback
                        if (!found) {
                ...
                WidgetCacheManager.triggerPrefetch(context, isLandscape = true)
